document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAidTypes() {
        ...
        tbody.innerHTML = data.map(a => `
            <tr>
                <td>${a.name}</td>
                <td><span class="badge" style="background:#f1f5f9;">${a.category}</span></td>
                <td>${a.amount} د.ل</td>
        ...
        if (e.key === 'Enter') {
            ...
            container.innerHTML = data.map(b => `
                <div style="background:var(--primary-light); padding:20px; border-radius:12px; margin-bottom:12px; border:1px solid var(--primary-dark);">
                    <h3>${b.name}</h3>
                    <p style="margin:8px 0;">الرقم الوطني: ${b.national_id}</p>
            ...
            document.getElementById('search-modal').classList.remove('hidden-section');
        }
    });

    // --- Form Handlers ---
    document.getElementById('btn-add-user')?.addEventListener('click', () => document.getElementById('user-modal').classList.remove('hidden-section'));
    document.getElementById('btn-add-beneficiary')?.addEventListener('click', () => document.getElementById('beneficiary-modal').classList.remove('hidden-section'));
    document.getElementById('btn-new-distribution')?.addEventListener('click', async () => {
        ...
        document.getElementById('distribution-modal').classList.remove('hidden-section');
    });
    ...
    addUserForm.addEventListener('submit', async (e) => {
        ...
        if (res.ok) { alert('تم الحفظ بنجاح'); document.getElementById('user-modal').classList.add('hidden-section'); loadUsers(); }
    });
    ...
    addBeneficiaryForm.addEventListener('submit', async (e) => {
        ...
        const result = await res.json();
        if (res.ok) { alert(`تم الحفظ. درجة الاحتياج: ${result.needScore}`); document.getElementById('beneficiary-modal').classList.add('hidden-section'); loadBeneficiaries(); }
    ...
    newDistributionForm.addEventListener('submit', async (e) => {
        ...
        const result = await res.json();
        if (res.ok) { alert(result.message); document.getElementById('distribution-modal').classList.add('hidden-section'); loadStats(); }
        else { alert(result.error); }
    });
    ...
    closeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById(mId).classList.add('hidden-section');
        });
    });

    window.addEventListener('click', (e) => { if (e.target.classList.contains('modal')) e.target.classList.add('hidden-section'); });
});
